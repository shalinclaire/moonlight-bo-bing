...
const PAGE_VERSION="20260921-25";
let currentNickname=(localStorage.getItem("bobing_nickname")||"").trim();
let currentPhone=(localStorage.getItem("bobing_phone")||"").trim();
...
function openNameModal(){nameInput.value=currentNickname;phoneInput.value=currentPhone;shopCodeInput.value=currentShopCode;nameModal.classList.add("open");setTimeout(()=>nameInput.focus(),80)}
function updateParticipant(){userName.textContent=currentNickname||"填写信息";if(!currentNickname||!/^1[3-9]\d{9}$/.test(currentPhone)||!currentShopCode)openNameModal()}
...
async function requestPlay(){
  const response=await fetch(`${SUPABASE_URL}/rest/v1/rpc/play_bobing_v25`,{method:"POST",headers:{apikey:SUPABASE_KEY,Authorization:`Bearer ${SUPABASE_KEY}`,"Content-Type":"application/json"},body:JSON.stringify({p_nickname:currentNickname,p_phone:currentPhone,p_shop_code:currentShopCode,p_device_id:deviceId,p_page_version:PAGE_VERSION})});
...
function roll(){
  ...
  if(!currentNickname||!/^1[3-9]\d{9}$/.test(currentPhone)||!currentShopCode){openNameModal();return}
  playDiceSound();
...
nameForm.addEventListener("submit",event=>{event.preventDefault();const value=nameInput.value.trim(),phone=phoneInput.value.replace(/\D/g,""),shopCode=shopCodeInput.value.trim().toUpperCase();if(!value||!/^1[3-9]\d{9}$/.test(phone)||!shopCode){phoneInput.setCustomValidity(/^1[3-9]\d{9}$/.test(phone)?"":"请输入正确的11位手机号码");phoneInput.reportValidity();return}phoneInput.setCustomValidity("");currentNickname=value;currentPhone=phone;currentShopCode=shopCode;localStorage.setItem("bobing_nickname",value);localStorage.setItem("bobing_phone",phone);localStorage.setItem("bobing_shop_code",shopCode);nameModal.classList.remove("open");updateParticipant();showToast(`欢迎你，${value}`)});
